...
const listSuggestedItemsForUserCache = `-- name: ListSuggestedItemsForUserCache :many
SELECT
  i.id,
  (1.0 - MIN(e.embedding <=> c.centroid))::double precision AS similarity,
  EXP(
...
type ListSuggestedItemsForUserCacheRow struct {
	ID         uuid.UUID `json:"id"`
	Similarity float64   `json:"similarity"`
	Freshness  float64   `json:"freshness"`
	Score      float64   `json:"score"`
...
func (q *Queries) ListSuggestedItemsForUserCache(ctx context.Context, userID uuid.UUID) ([]ListSuggestedItemsForUserCacheRow, error) {
	rows, err := q.db.Query(ctx, listSuggestedItemsForUserCache, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []ListSuggestedItemsForUserCacheRow
	for rows.Next() {
		var i ListSuggestedItemsForUserCacheRow
		if err := rows.Scan(
			&i.ID,
			&i.Similarity,
			&i.Freshness,
