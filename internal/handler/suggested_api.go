...
// ListUserSuggestedItems returns suggested items based on user likes.
...
// @Security     BearerAuth
// @Router       /api/suggested [get]
func (h *Handler) ListUserSuggestedItems(w http.ResponseWriter, r *http.Request) {
	...
	resp, err = h.Service.ListUserSuggestedItems(r.Context(),
		repository.ListSuggestedItemsByUserParams{
			UserID: userID,
			Limit:  params.Limit,
	...
	if err != nil {
		...
		if errors.As(err, &serviceErr) {
			...
			http.Error(w, "failed to list suggested items", http.StatusBadRequest)
			return
		}
	}
